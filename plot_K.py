...
Visualisation of the effect of the varying carrying capacity values in 
Lotka-Volterra dynamics on the host population
...
D_K_df = pd.DataFrame(dict(K1 = K_1_ij, K2 = K_2_ij, deceased = dead_total))
D_to_Ks = D_K_df.pivot("K1", "K2", "deceased")

dead_fraction = [di/dt for di,dt in zip(dead_inf,dead_total)]

Df_K_df = pd.DataFrame(dict(K1 = K_1_ij, K2 = K_2_ij, deceased_inf = dead_fraction))
Df_to_Ks = Df_K_df.pivot("K1", "K2", "deceased_inf")
...
ax = sns.heatmap(data = Df_to_Ks, cbar_kws={'label': 'deceased through infection'})
plt.show()
